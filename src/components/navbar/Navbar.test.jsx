...
const Navbar = ({ theme, toggleTheme }) => {
  ...
  return (
    <nav className={`navbar shadow-lg sticky top-0 ${theme === 'light' ? 'bg-white text-black' : 'bg-gray-800 text-white'} transition-colors duration-300`}>
      <div className="container mx-auto flex justify-between items-center p-4">
        {/* Website Name */}
        <h1 className="text-2xl font-bold uppercase text-green-500">ALapparel</h1>
        
        {/* Links for Desktop */}
        <ul className="hidden md:flex space-x-4 uppercase text-green-600">
        ...
        {/* Theme Toggle Button */}
        <button onClick={toggleTheme} className="ml-4 bg-transparent toggle-theme" aria-label={`Switch to ${theme === 'light' ? 'dark' : 'light'} mode`}>
          {theme === 'light' ? '🌙' : '☀️'}
        </button>
      ...
      {/* Links for Mobile */}
      {isMenuOpen && (
        <ul className={`md:hidden p-4 space-y-2 ${theme === 'light' ? 'bg-white text-green-500' : 'bg-gray-800 text-green-500'} transition duration-300 ease-in-out`}>
